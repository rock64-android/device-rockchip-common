...
#if (defined(__GNUC__) || defined(__ICC)) && !defined(__ARMCC_VERSION)
  #include <sys/types.h>
#endif
...
#if (!defined(__sun) && !defined(__linux) && !defined(__unix)) || defined(__ICC)
...
**
*/
#if !defined (__arm) && !defined (__ICC) && !defined(__GNUC__) || defined(REGI) 
#define __align(x)
#endif

/*
** Other than the below listed macros, if the user modules need specific requirements which are very specific to
** a particular compiler and not generic, then the user modules are expected to know the details of that option
** and hence user module code has to take care of using under proper defines for different compiler support.
** For the same, user module code can use the defines __ICC, __GNUC__, __ARMCC_VERSION, etc
** (predefined macros by each supported compiler to uniquely identify themselves)
**
** For any assembly or architecture specific variant code, use __arm and __x86 to differentiate. 
** Pls dont misplace the __arm & __x86 in place of the below macros  
*/
#if defined(__ICC) || (defined(__GNUC__) && !defined(REGI))
#define __align(align_size) __attribute__((aligned(align_size)))
#define __weak __attribute__ ((weak))
//#define __packed __attribute__ ((packed))
#define __NOP() asm volatile ( "nop" )
#define __forceinline __attribute__((always_inline)) 
#define __SECTION(section_name) __attribute__((section(section_name)))
#define __SECTION_ATTR(section_name,attr) __attribute__((section(section_name)))
#elif defined(__ARMCC_VERSION)
#define __SECTION(section_name) __attribute__((section(section_name)))
#define __SECTION_ATTR(section_name,attr) __attribute__((section(section_name),attr))
#endif
